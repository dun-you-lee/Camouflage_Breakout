...
)
{
	...
	if (jd->sz_pool >= ndata) {
		jd->sz_pool -= ndata;			// "sz_pool -  ndata)" bytes left unused after allocation
		rp = (char*)jd->pool;			/* Get start of available memory pool */
		jd->pool = (void*)(rp + ndata);	/* Allocate requierd (ndata) bytes */
	}
...
/*-----------------------------------------------------------------------*/
/* Load all blocks in an MCU  (Minimum Coded Unit) into working buffer                         */
/*-----------------------------------------------------------------------*/
...
JRESULT jd_prepare (
	...
	size_t (*infunc)(JDEC*, uint8_t*, size_t),	/* JPEG strem input function (bytes reading) */
...
)
{
	...
	jd->inbuf = seg = alloc_pool(jd, JD_SZBUF);		/* Allocate stream input buffer */
		// take JD_SZBUF bytes from "jd->pool" to "jd->inbuf"
		// "seg" stands for "jd>inbuf"
	if (!seg) return JDR_MEM1;

	ofs = marker = 0;		/* Find SOI marker */
	do {
		if (jd->infunc(jd, seg, 1) != 1) return JDR_INP;	/* Err: SOI was not detected */
		// if the number of bytes read (nbyte=1) is NOT 1 //
		if(((marker & 0xFF) == 0xFF) && (seg[0] != 0xD8)) return JDR_INP;	/* Err: SOI was not detected */
		marker = marker << 8 | seg[0];
		ofs++;
	} while (marker != 0xFFD8);		// Start of jpg file marker must be "FF D8"
	...
	for (;;) {				/* Parse JPEG segments */
		...
		case 0xC0:	/* SOF0 (baseline JPEG) */
			// Obtain: jd->width, jd->height, jd->ncomp, jd->msx, jd->msy, jd->qtid[]
			if (len > JD_SZBUF) return JDR_MEM2;
			if (jd->infunc(jd, seg, len) != len) return JDR_INP;	/* Load segment data */

			jd->width = LDB_WORD(&seg[3]);		/* Image width in unit of pixel */
			jd->height = LDB_WORD(&seg[1]);		/* Image height in unit of pixel */
			jd->ncomp = seg[5];					/* Number of color components: 1:grayscale, 3:color */
		...
		case 0xDD:	/* DRI - Define Restart Interval */
			...
			jd->nrst = LDB_WORD(seg);	/* Get restart interval (MCUs: (Minimum Coded Unit)) */
			break;

		case 0xC4:	/* DHT - Define Huffman Tables */
			// Obtain: jd->huffbits[][], huffcode[][], huffdata[][]
			if (len > JD_SZBUF) return JDR_MEM2;
			if (jd->infunc(jd, seg, len) != len) return JDR_INP;	/* Load segment data */
		...
		case 0xDA:	/* SOS - Start of Scan */
			// Obtain: jd->workbuf, jd->mcubuf, jd->dctr, jd->dptr
	//>>>>>>>>>>>###################################################################
			if (len > JD_SZBUF) return JDR_MEM2;
			if (jd->infunc(jd, seg, len) != len) return JDR_INP;	/* Load segment data */
			...
			jd->dptr = seg + ofs - (JD_FASTDECODE ? 0 : 1);	/* Current data read ptr */
		...
		default:	/* Unknown segment (comment, exif or etc..) */
			/* Skip segment data (null pointer specifies to REMOVE (skip) data from the stream) */
